% MIRT2D_REGISTER The main function for non-rigid registration 
% of a single pair of 2D images using cubic B-spline
...
%               listed in mirt2D_similarity. You can easily add your own
...
%
%       .X      a 3D matrix of final B-spline control points positions
%               The 3rd dimension size is 2, it indicates the coordinate x and y
%
%       .okno   window size/spacing between the control points (equal to the initial main.okno).
%
...
%
%   See also mirt2D_registration, mirt2Dgroup_sequence, mirt2Dgroup_frame,
%   MIRT_GUI, mirt3D_register
...
if (numel(size(im))~=numel(size(refim)))|| (size(im,1)~=size(refim,1)) || (size(im,2)~=size(refim,2))
     im=imresize(im,[dim(1) dim(2)]); 
end
% 
% dim=size(refim);
% 
...
tic
disp('Lancement du recalage basé sur les B-splines');
...
main.siz=[(main.mg-3)*main.okno (main.ng-3)*main.okno];

main.X=cat(3,x,y);  % Put x and y control point positions into a single mg x ng x 2 3Dmatrix
main.Xgrid=main.X;  % save the regular grid (used for regularization to find the displacements)
...
Rec=figure('name','Progression du recalage','position',[400 250 700 350],'resize','off'...
...
for level=1:main.subdivide
    ...
    % current image size
    main.siz=[(main.mg-3)*main.okno (main.ng-3)*main.okno];
    main.K=mirt2D_initK([main.mg main.ng]); % Init Laplacian eigenvalues (used for regularization)
    
    main.refimsmall=imresize(refim,main.siz,'bicubic'); % resize images
    main.refimsmall(main.refimsmall<0)=0;  
    main.refimsmall(main.refimsmall>1)=1;
    
    imsmall=imresize(im,main.siz,'bicubic'); 
    imsmall(imsmall<0)=0;
    imsmall(imsmall>1)=1;
    
    [gradx, grady]=gradient(imsmall);
    main.imsmall=cat(3,imsmall, gradx,grady); % concatenate the image with its gradients
    % main.imsmall is a 3D matrix - a concatination of the image and its x
    % and y gradients at the given hierarchical level. The reason to
    % concatinate them together is to save time later on image interpolation
    % using mirt2D_mexinterp. The image and its gradients have to be
    % interpolated at the same coordinates, which can be done more
    % efficiently then interpolating each of them individially.
    
    
    % a single level 2D non-rigid image registration
    [main.X, result]=mirt2D_registration(main.X, main, optim);
    
    % if the sublevel is not last prepair for the next level
    if level<main.subdivide,
        main.X=mirt2D_subdivide(main.X, 1);
...
disp('Fin du recalage basé sur les B-splines')
disptime(toc);
% im=imresize(im,dim);
% refim=imresize(refim,dim);
