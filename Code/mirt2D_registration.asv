...
while (abs(fchange)>optim.fundif) && (iter<optim.maxsteps)
        ...
        if ((fp-f)>0),
            ...
            if main.single
                subplot(1,3,1,'align'), imshow(main.refimsmall); title('Image Cible'); drawnow;
                subplot(1,3,2,'align'), imshow(im); title('Progression du recalage'); drawnow;
                subplot(1,3,3,'align'), mirt2D_meshplot(X(:,:,1),X(:,:,2));  title('Grille de transformation'); drawnow;
            end
             
            % show progress 
           disp(['Sim. ' upper(main.similarity) ' ' num2str(f) ' Dif. ' num2str(fchange) ' Niv. ' num2str(main.level) ' Iter. ' num2str(iter) ' Gamma ' num2str(optim.gamma)]);
        end;
      iter=iter+1;
end
